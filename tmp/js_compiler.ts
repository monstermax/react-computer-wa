...
const sourceCodeFilepath = process.argv[2];

if (!sourceCodeFilepath || !fs.existsSync(sourceCodeFilepath)) {
    console.log(`Missing source code file argument`);
    process.exit();
}

const testCode = fs.readFileSync(sourceCodeFilepath).toString()
...
const asmCodeFilepath = sourceCodeFilepath + '.asm';
fs.writeFileSync(asmCodeFilepath, asmText);
