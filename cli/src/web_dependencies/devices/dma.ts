...
import { high16, low16, U16, U8 } from "../../lib/lib_numbers";
...
export type DmaDeviceParams = {
    ...
    vendor?: string;
    model?: string;
    devicesRef: Map<number, IoDevice>;
...
export class DmaDevice extends IoDevice {
    static type = 'system';
    private devicesRef: Map<number, IoDevice>;
    ...
    write(port: u8, value: u8): void {
        ...
        switch (port) {
            ...
            case 0x07 : { // DMA_DATA - writeData from device to RAM
                const device = this.devicesRef.get(this.selectedIo);
    ...
    reset() {
        ...
        this.sourceStartAddress = U16(0); // disk address
        this.sourceEndAddress = U16(0); // disk address
        this.targetStartAddress = U16(0); // ram address
    }
